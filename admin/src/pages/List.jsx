  ...
  const removeProduct = async (id) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Optimistically update the list without fetching again
        ...
        {/* ------- Product List Items -------- */}
        {
          !loading && list.map((item, index) => (
            <div className='grid grid-cols-[1fr_3fr_1fr] md:grid-cols-[1fr_3fr_1fr_1fr_1fr] items-center gap-2 py-1 px-2 border text-sm' key={index}>
              <img className='w-12' src={item.image && item.image[0] ? item.image[0] : 'placeholder_image_url'} alt={item.name || 'Product Image'} />
              <p>{item.name}</p>
              <p>{item.category}</p>
